...
# 키 내꺼
Servicekey = 'S9hkvJj0C%2FrTlURHqlFJmoHup876DkXnj25KJ28UCP8aX%2FyZe7BClbq8txGLm%2FXe5TMoUl4ADUHnmwGRwTG7dQ%3D%3D'
    ...
    if jsonData['getgmgcourseinfo']['header']['code'] == '00':
        if jsonData['getgmgcourseinfo']['item'] =='':
            ...
            for item in jsonData['getgmgcourseinfo']['item']:
                ...
                result.append([seq, course_nm, gugan_nm, gm_range, gm_degree, start_pls, start_addr, middle_pls, middle_adr,
...
def main():
    ...
    if len(result) >0:
        # 파일저장 csv
        columns = ['seq', 'course_nm', 'gugan_nm', 'gm_range', 'gm_degree', 'start_pls', 'start_addr', 'middle_pls', 'middle_adr',
                        'end_pls', 'end_addr', 'gm_course', 'gm_text']
        ...
        # DB 저장
        connection = pymysql.connect(host='localhost',user='root',password='1234',db='crwaling_data')
