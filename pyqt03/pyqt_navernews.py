...
class qTemplate(QWidget):
    ...
    def btnSearchClicked(self) -> None:     # 슬롯(이벤트핸들러)
        ...
        self.lblStatus2.setText(f'저장할 데이터 > {len(self.saveResult)}개')

        if curr >= 1000:
            self.btnNext.setDisabled(True) # 다음 버튼 비활성화
        else:
    ...
    def getPostData(self, post):
        ...
        temp.append({'title':title, 'description':description, 'originallink':originallink, 'link':link, 'pubDate':pubDate}) # pubDate 빠진거 추가
    ...
    def getNaverSearch(self, keyword, search, start, display):
        ...
        # 네이버 인증 추가
        req.add_header('X-Naver-Client-Id', 'nqEoZxpJff13YQphw7EJ')
        req.add_header('X-Naver-Client-Secret', 'qbaq1YzQwm')
